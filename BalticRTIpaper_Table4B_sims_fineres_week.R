...
### Calculate CPUE at the resolution of 0.2 degrees longitude x 0.1 degrees latitude and week
...
updateCOD<- array(dim=c(43,26,53))
updatePLE<- array(dim=c(43,26,53))
updateCOMBINED<- array(dim=c(43,26,53))
...
newinfo <- array(dim=c(43,26,53,2))
...
for (z in 1:1000)
{
  ...
  total_catchPLE<-0
  cumulateRTI<-0
  while(cumulateRTI<2030) # the following loop goes on as long as the unequality is fulfilled
