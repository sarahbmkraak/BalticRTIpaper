...
### Calculate CPUE at the resolution of 1 degrees longitude x 0.5 degrees latitude and week
...
updateCOD<- array(dim=c(9,6,53))
updatePLE<- array(dim=c(9,6,53))
updateCOMBINED<- array(dim=c(9,6,53))
...
for (z in 1:1000)
{
  ...
  total_catchPLE<-0
  cumulateRTI<-0
  while(cumulateRTI<2150) # the following loop goes on as long as the unequality is fulfilled
