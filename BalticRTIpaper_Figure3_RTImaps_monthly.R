#### first the baseline maps and then the update by month maps
library(lubridate)
library(maps)
library(mapdata)
library(mapplots)
...
# make the map based on data of 2014-2017 for month 1 (baseline RTIs)
# limits of maps
xlim <- c(9.5,18)
ylim <- c(54,55.5)
...
basemap(xlim=c(9.5,18.5),ylim=ylim,xaxt='n',yaxt='n',xlab='Longitude (°E)',ylab='Latitude (°N)',main=paste('RTI tariffs for month 1 of 2018, based on data from 2014-2017'), bg='white')
...
# now for 52 months, in month 1 baseline RTIs are valid; update[,,2] is RTIs for month 2 updated by month 1 info, etc.
...
breaks <- c(0,1.1,2.1,3.1,4.1,5.1,6.1)
basemap(xlim=c(9.5,18.5),ylim=ylim,xaxt='n',yaxt='n', main=paste('RTI tariffs for month',k+1,'updated by data from month',k), bg='white')
