...
### we need lat and lon columns for the RTI resolution of 1 degrees longitude x 0.5 degrees latitude
maps_data$lon_RTI<-round((maps_data$lon_fine),0)
maps_data$lat_RTI<-round(maps_data$lat_fine*2,0)/2
...
### Calculate CPUE at the resolution of 1 degrees longitude x 0.5 degrees latitude and month
...
CPUE_grid_COD_4.4<-make.grid(aggCOD_under_4.4$x, aggCOD_under_4.4$y, aggCOD_under_4.4$NEW_CPUE_COD, byx=1 , byy=0.5 , xlim=xlim, ylim=ylim, fun = mean)
...
CPUE_grid_PLE<-make.grid(aggPLE_under_4.4$x, aggPLE_under_4.4$y, aggPLE_under_4.4$NEW_CPUE_PLE, byx=1 , byy=0.5 , xlim=xlim, ylim=ylim, fun = mean)
...
updateCOD<- array(dim=c(9,6,12))
updatePLE<- array(dim=c(9,6,12))
updateCOMBINED<- array(dim=c(9,6,12))
...
### here the relative CPUEs are calculated 
  ...
  CPUE_grid_COD_1<-make.grid(aggCOD_under_4.4_2018_1$x, aggCOD_under_4.4_2018_1$y, aggCOD_under_4.4_2018_1$NEW_CPUE_COD, byx=1 , byy=0.5 , xlim=xlim, ylim=ylim, fun = mean)
  ...
  CPUE_grid_PLE_1<-make.grid(aggPLE_under_4.4_2018_1$x, aggPLE_under_4.4_2018_1$y, aggPLE_under_4.4_2018_1$NEW_CPUE_PLE, byx=1 , byy=0.5 , xlim=xlim, ylim=ylim, fun = mean)
...
for (z in 1:1000)
{
  ...
  total_catchPLE<-0
  cumulateRTI<-0
  while(cumulateRTI<2300) # the following loop goes on as long as the unequality is fulfilled
  { # choose each time a random cell in a random month
    ...
    a <-   round(runif(1,min=1.5,max=9.49)) # a and b are coordinates of a random cell
    b <-   round(runif(1,min=1.5,max=5.49))
